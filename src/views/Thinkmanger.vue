        ...
        <div class='register_modal_content'>
            <i-form  :model="auModel_data" :rules="ruleInline" :label-width="80" >
                ...
                <Form-item prop="member" label="人数">
                    <i-input  v-model="auModel_data.num" placeholder="请输入内容"/>
                 </Form-item>
                    </i-col>
                </Row>
                <Row>
                    <i-col span='23'>
                <Form-item prop="discrt" label="描述">
                    <i-input  v-model="auModel_data.discrt" placeholder="请输入内容"/>
                 </Form-item>
                    </i-col>
                </Row>
                    <i-col span='23'>
                <Form-item prop="idetime" label="汇报时间：">
                     <Date-picker :disabled="enable" type="date" @on-change="changeE(auModel_data.idetime)" v-model="auModel_data.idetime" format="yyyy年MM月dd日" placeholder="选择入党时间" style="width: 100%" ></Date-picker>
                 </Form-item>
                    </i-col>
                </Row>
            </i-form>
...
export default {
    ...
    changeE(e){
      this.auModel_data.idetime=changeDate(e)
    },
    handlePageSize(val) {
    ...
    async del(params){
        let res= await v_axios('party/delilist','post',params.row,"")
        if(res){
             this.$Notice.success({
                title:'成功',
                desc:'删除成功',
    ...
    async  getPartylist(){
        ...
        let res= await v_axios('party/getilist','post',this.options,"")
        this.data=res
   ...
   async submit(param){
     var options=this.auModel_data
      if(param=='新增思想'){
            let res= await v_axios('party/addilist','post',options,"")
            if(res){
              this.$Notice.success({
                title:'成功',
                desc:'新增成功',
        ...
        let res= await v_axios('party/updteilist','post',options,"")
            if(res){
              this.$Notice.success({
                title:'成功',
                desc:'修改成功',
